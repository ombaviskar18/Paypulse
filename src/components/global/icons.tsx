...
type IconType = {
    [key: string]: (props: LucideProps & { className?: string; width?: number | string; height?: number | string }) => JSX.Element;
};

const Icons: IconType = {
    icon: (props: LucideProps & { className?: string; width?: number | string; height?: number | string }) => {
        // Extract size from className or use defaults
        const extractSize = (className?: string, defaultSize: number = 24) => {
            if (!className) return defaultSize;
            // Try to match w-* or h-* classes (Tailwind: w-5 = 20px, w-6 = 24px, etc.)
            const widthMatch = className.match(/w-(\d+)/);
            const heightMatch = className.match(/h-(\d+)/);
            
            if (widthMatch) return parseInt(widthMatch[1]) * 4;
            if (heightMatch) return parseInt(heightMatch[1]) * 4;
            return defaultSize;
        };
        
        let width = props.width ? (typeof props.width === 'string' ? parseInt(props.width) : props.width) : extractSize(props.className, 24);
        let height = props.height ? (typeof props.height === 'string' ? parseInt(props.height) : props.height) : extractSize(props.className, 24);
        
        // If only one dimension is set via className, use it for both to maintain square aspect ratio
        if (props.className) {
            const hasWidth = props.className.includes('w-');
            const hasHeight = props.className.includes('h-');
            if (hasWidth && !hasHeight) height = width;
            if (hasHeight && !hasWidth) width = height;
        }
