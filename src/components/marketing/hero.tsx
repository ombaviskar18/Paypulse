...
const Hero = () => {
    return (
        <div id="home" className="flex flex-col items-center text-center w-full max-w-5xl my-24 mx-auto z-40 relative">
            <Container delay={0.0}>
                <div className="pl-2 pr-1 py-1 rounded-full border border-foreground/10 hover:border-foreground/15 backdrop-blur-lg cursor-pointer flex items-center gap-2.5 select-none w-max mx-auto">
                    ...
                    <span className="inline-flex items-center justify-center gap-2 animate-text-gradient animate-background-shine bg-gradient-to-r from-[#b2a8fd] via-[#8678f9] to-[#c7d2fe] bg-[200%_auto] bg-clip-text text-sm text-transparent">
                        Built for Stellar • Offline-First
            ...
            <BlurText
                word={"PayPulse Stellar\nOffline  Payments via Bluetooth"}
            ...
            <Container delay={0.1}>
                <p className="text-sm sm:text-base lg:text-lg mt-4 text-accent-foreground/60 max-w-2xl mx-auto">
                    Send and receive XLM or PAYPULSE tokens without internet using secure Bluetooth LE. Transactions auto-sync to the Stellar blockchain when back online. Built with strong cryptography, encrypted storage, and biometric authentication.
                </p>
            </Container>
            <Container delay={0.2}>
                <div className="flex items-center justify-center md:gap-x-6 mt-8">
                    <Button asChild size="lg">
                        <Link href="https://drive.google.com/drive/folders/1ReArGGtvMjww71H6S0vrPsevsW99cSCS?usp=drive_link">
                            Try the Demo
                        </Link>
                    </Button>
            ...
            <Container delay={0.3}>
                <div className="relative mx-auto max-w-7xl rounded-xl lg:rounded-[32px] border border-neutral-200/50 p-2 backdrop-blur-lg border-neutral-700 bg-neutral-800/50 md:p-4 mt-12 overflow-hidden">
                    <div className="absolute top-1/4 left-1/2 -z-10 gradient w-3/4 -translate-x-1/2 h-1/4 -translate-y-1/2 inset-0 blur-[10rem]"></div>
                    <div className="rounded-lg lg:rounded-[24px] border border-neutral-700 bg-black overflow-hidden relative">
                        <video
                            src="/images/hero.mp4"
                            autoPlay
                            muted
                            loop
                            playsInline
                            className="w-full h-full rounded-lg lg:rounded-[20px] object-cover"
                        />
                    </div>
                </div>
            </Container>
